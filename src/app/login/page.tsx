import Image from 'next/image'
import Link from 'next/link'
import React from 'react'
import { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Zenbowls',
}

const LoginPage = () => {
    return (
        <div className='p-4 h-[calc(100vh-6rem)] md:[calc(100vh-9rem)] flex items-center justify-center'>
            {/* BOX */}
            <div className='h-full shadow-2xl rounded-md flex flex-col md:flex-row md:h-[70%] md:w-full lg:w-[60%] 2xl:w-[50%]'>
                {/* IMAGECOHNTAINER */}
                <div className='relative h-1/3 w-full md:h-full md:w-1/2'>
                    <Image src="/Login-img.jpg" alt='' fill className='object-cover'/>
                </div>
                {/* FORM CONTAINER */}
                <div className='p-10 flex flex-col gap-8 md:w-1/2'>
                    <h1 className='font-bold text-xl xl:text-3xl'>Welcome</h1>
                    <p>Log into your account or create a new one</p>
                    <button className='flex gap-4 p-4 ring-1 ring-gray-100 rounded-lg hover:bg-gray-100'>
                    <Image src="google.svg" alt='' width={20} height={20} className='object-contain'/>
                    <span>Sign in with Google</span>
                    </button>
                    <button className='flex gap-4 p-4 ring-1 ring-gray-100 rounded-lg hover:bg-gray-100'>
                    <Image src="facebook.svg" alt='' width={20} height={20} className='object-contain'/>
                    <span>Sign in with Facebook</span>
                    </button>
                    <p className='text-sm'>Have a problem? <Link href="/" className='underline'>Contact us</Link></p>
                </div>
            </div>
        </div>
    )
}

export default LoginPage